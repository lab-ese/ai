...
def best_first_search(initial, goal):
    start_tuple = tuple(tuple(row) for row in initial)
    goal_tuple = tuple(tuple(row) for row in goal)
    pq = [(get_manhattan_distance(initial, goal), start_tuple, [])]
    visited = {start_tuple}
    while pq:
        h, current, path = heapq.heappop(pq)
        if current == goal_tuple: return path + [current]
        for neighbor in get_neighbors(current):
            if neighbor not in visited:
                visited.add(neighbor)
                heapq.heappush(pq, (get_manhattan_distance(neighbor, goal), neighbor, path + [current]))
    return None
...
if __name__ == "__main__":
    print("--- BEST FIRST SEARCH: 8-PUZZLE ---")
    start = get_input()
